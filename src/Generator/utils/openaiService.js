...
const callGemini = async (context, apiKey) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.log('Gemini API Error - Status:', response.status, 'Status Text:', response.statusText);
    ...
    logger.log('Gemini API Success - Response received');
...
const callOpenRouter = async (context, apiKey) => {
  ...
  try {
    logger.log('Calling OpenRouter API with model:', model);
    ...
    if (!response.ok) {
      ...
      logger.log('OpenRouter API Error - Status:', response.status, 'Status Text:', response.statusText);
    ...
    const data = await response.json();
    logger.log('OpenRouter API Success - Response received');
...
const callOpenAIProvider = async (context, apiKey) => {
  ...
  try {
    logger.log('Calling OpenAI API with model:', model);
    ...
    if (!response.ok) {
      ...
      logger.log('OpenAI API Error - Status:', response.status, 'Status Text:', response.statusText);
    ...
    const data = await response.json();
    logger.log('OpenAI API Success - Response received');
    const aiMessage = data.choices[0].message.content;
...
export const testApiKey = async (apiKey, provider = 'gemini') => {
  try {
    logger.log('Testing API key for provider:', provider);
    ...
    if (provider === 'gemini') {
      ...
      logger.log('Gemini test response status:', response.status);
      ...
      if (response.ok) {
        const data = await response.json();
        logger.log('Gemini test success:', data);
        return true;
      }
      ...
      if (response.status === 400) {
        try {
          ...
          logger.log('Gemini 400 response:', errorData);
      ...
      if (response.status === 403) {
        const errorData = await response.json();
        logger.log('Gemini 403 error:', errorData);
        return false;
      }
      ...
      logger.log('OpenRouter test response status:', response.status);
      return response.ok;
    } else {
      ...
      logger.log('OpenAI test response status:', response.status);
      return response.ok;
    }
  } catch (error) {
